"""Lightweight console/file logger facade used by training utilities."""

import os
import logging
import datetime
from typing import Dict
from PIL import Image

from utils.time_estimator import TimeEstimator


def tensor_to_numpy(image):
    image_np = (image.numpy() * 255).astype('uint8')
    return image_np


def detach_to_cpu(x):
    return x.detach().cpu()


def fix_width_trunc(x):
    return ('{:.9s}'.format('{:0.9f}'.format(x)))


class TrainingLogger:
    def __init__(self, run_dir, py_logger: logging.Logger, *, enabled_tb=True, git_info_enabled=False):
        """
        """
        self.run_dir = run_dir
        self.py_log = py_logger

        # 
        git_info = "disabled"
        if git_info_enabled:
            git_info = self._get_git_info()
        self.log_string("git", git_info)

        self.time_estimator: TimeEstimator = None

    def _get_git_info(self) -> str:
        try:
            from pathlib import Path
            import git

            repo = git.Repo(Path(__file__).resolve().parents[1], search_parent_directories=True)

            # 分支名
            try:
                branch = repo.active_branch.name
            except Exception:
                try:
                    name = repo.git.rev_parse("--abbrev-ref", "HEAD")
                    branch = name if name != "HEAD" else os.environ.get("GIT_BRANCH", "detached")
                except Exception:
                    branch = os.environ.get("GIT_BRANCH", "unknown")

            # 
            try:
                commit = repo.git.rev_parse("--short", "HEAD")
            except Exception:
                commit = "unknown"

            return f"{branch} {commit}"
        except Exception as e:
            print(f"Failed to fetch git info ({e}). Defaulting to None")
            return "None"

    # ----------- Logging methods -----------

    def log_scalar(self, tag, x, it):
        return None

    def log_metrics(self, exp_id, prefix, metrics: Dict, it):
        msg = f"{exp_id}-{prefix} - it {it:6d}: "
        metrics_msg = ""
        for k, v in sorted(metrics.items()):
            self.log_scalar(f"{prefix}/{k}", v, it)
            metrics_msg += f"{k:>10}:{v:.7f},\t"

        if self.time_estimator is not None:
            self.time_estimator.update()
            avg_time = self.time_estimator.get_and_reset_avg_time()
            est = self.time_estimator.get_est_remaining(it)
            est = datetime.timedelta(seconds=est)
            remaining_str = (
                f"{est.days}d {est.seconds // 3600}h"
                if est.days > 0
                else f"{est.seconds // 3600}h {(est.seconds % 3600) // 60}m"
            )
            eta = datetime.datetime.now() + est
            eta_str = eta.strftime("%Y-%m-%d %H:%M:%S")
            time_msg = (
                f"avg_time:{avg_time:.3f},remaining:{remaining_str},eta:{eta_str},\t"
            )
            msg = f"{msg} {time_msg}"

        msg = f"{msg} {metrics_msg}"
        self.py_log.info(msg)

    def log_image(self, stage_name, tag, image, it):
        image_dir = os.path.join(self.run_dir, f"{stage_name}_images")
        os.makedirs(image_dir, exist_ok=True)
        Image.fromarray(image).save(os.path.join(image_dir, f"{tag}_{it}.png"))

    def log_string(self, tag, x):
        self.py_log.info(f"{tag} - {x}")

    def debug(self, x): self.py_log.debug(x)
    def info(self, x): self.py_log.info(x)
    def warning(self, x): self.py_log.warning(x)
    def error(self, x): self.py_log.error(x)
    def critical(self, x): self.py_log.critical(x)
