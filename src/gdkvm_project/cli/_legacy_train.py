...
from gdkvm_project.configs import validate_project_config
...
def _label_valid_stats(hist: dict[str, int], total_frames: int) -> dict[str, float | int]:
    if not hist:
        return {
            "supervised_frames_mean": float(total_frames),
            "supervised_frames_min": int(total_frames),
            "supervised_frames_max": int(total_frames),
            "label_sparsity": 0.0,
        }
    count = sum(int(v) for v in hist.values())
    weighted = sum(int(k) * int(v) for k, v in hist.items())
    min_frames = min(int(k) for k in hist)
    max_frames = max(int(k) for k in hist)
    mean = float(weighted) / max(float(count), 1.0)
    return {
        "supervised_frames_mean": mean,
        "supervised_frames_min": int(min_frames),
        "supervised_frames_max": int(max_frames),
        "label_sparsity": 1.0 - mean / max(float(total_frames), 1.0),
    }
...
def _infer_temporal_access(cfg: DictConfig) -> str:
    ...
    default = "full_window" if model_name in {"dpfr", "dual_prompt_flow_refinement"} else "recurrent"
    return str(
        cfg.get(
            "temporal_access",
...
def _experiment_protocol_summary(cfg: DictConfig, stage_cfg: DictConfig, world_size: int) -> dict:
    ...
    return {
        ...
        "amp": bool(stage_cfg.get("amp", False)),
...
def _runtime_summary() -> dict:
    return {
        "entrypoint": "gdkvm_project.cli.train",
...
def _log_data_flow_summary(
    *,
    ...
    supervision_stats = _label_valid_stats(train_probe.get("label_valid_hist", {}), int(stage_cfg.seq_length))
    ...
    params = {
        ...
        "data/supervised_frames_mean_sample": supervision_stats["supervised_frames_mean"],
        "data/label_sparsity_sample": supervision_stats["label_sparsity"],
...
@hydra.main(version_base="1.3.2", config_path="../../../configs", config_name="gdkvm_echo")
def train(cfg: DictConfig):
    ...
    run_recorder.log_config(cfg)
    run_recorder.log_runtime(_runtime_summary())
    run_recorder.log_git()
    ...
    try:
        ...
        if main_process and stage in {"full", "final"} and mlflow_required and not mlflow_enabled:
            info_if_rank_zero("MLflow disabled for this run; local recorder artifacts remain enabled.")
        ...
        for it in range(total_iterations):
            ...
            smoke_skip_eval = bool(cfg.get("smoke_skip_eval", True)) and total_iterations <= 1
            final_eval = bool(getattr(cfg.eval_stage, "final_eval", True)) and not smoke_skip_eval
            if smoke_skip_eval and is_final_iter and main_process:
                log.info("Skipping final eval/test for one-iteration smoke run; set smoke_skip_eval=false to force it.")
        ...
        run_recorder.log_summary({"status": "finished", "exp_id": str(cfg.get("exp_id", "experiment"))})
        run_recorder.ensure_required_files()
        if mlflow_started:
        ...
        run_recorder.log_summary({"status": "failed", "exp_id": str(cfg.get("exp_id", "experiment"))})
        run_recorder.ensure_required_files()
        raise
