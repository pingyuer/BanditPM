...
SESSION_NAME="${SESSION_NAME:-geomaskformer_echo_search_$(date +%Y%m%d_%H%M%S)}"
LOG_DIR="${LOG_DIR:-logs/geomaskformer_echo_search_$(date +%Y%m%d_%H%M%S)}"
OUT_DIR="${OUT_DIR:-outputs/geomaskformer_echo_search_$(date +%Y%m%d_%H%M%S)}"
...
tmux new-session -d -s "${SESSION_NAME}" -n gpu0 \
  "cd '${PROJECT_DIR}' && echo '[\$(date +%F\\ %T)] START fullres_proposal' && $(declare -f run_one); COMMON_ARGS=($(printf '%q ' "${COMMON_ARGS[@]}")); PROJECT_DIR='${PROJECT_DIR}'; UV_BIN='${UV_BIN}'; DATASETS_ROOT='${DATASETS_ROOT}'; OUT_DIR='${OUT_DIR}'; run_one 0 geomaskformer_fullres_proposal_echo geomaskformer_fullres_proposal_echo '${LOG_DIR}/fullres_proposal_echo.log' 2>&1 | tee '${LOG_DIR}/fullres_proposal_echo.log'; echo '[\$(date +%F\\ %T)] END gpu0 queue'; exec bash"

tmux new-window -t "${SESSION_NAME}" -n gpu1 \
  "cd '${PROJECT_DIR}' && echo '[\$(date +%F\\ %T)] START cascade_refine' && $(declare -f run_one); COMMON_ARGS=($(printf '%q ' "${COMMON_ARGS[@]}")); PROJECT_DIR='${PROJECT_DIR}'; UV_BIN='${UV_BIN}'; DATASETS_ROOT='${DATASETS_ROOT}'; OUT_DIR='${OUT_DIR}'; run_one 1 geomaskformer_cascade_refine_echo geomaskformer_cascade_refine_echo '${LOG_DIR}/cascade_refine_echo.log' 2>&1 | tee '${LOG_DIR}/cascade_refine_echo.log'; echo '[\$(date +%F\\ %T)] END gpu1 queue'; exec bash"

echo "Started tmux session '${SESSION_NAME}'."
echo "Attach with: tmux attach -t ${SESSION_NAME}"
echo "Logs:"
echo "  ${PROJECT_DIR}/${LOG_DIR}/fullres_proposal_echo.log"
echo "  ${PROJECT_DIR}/${LOG_DIR}/cascade_refine_echo.log"
