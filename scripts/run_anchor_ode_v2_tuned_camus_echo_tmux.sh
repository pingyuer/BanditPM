...
SESSION_NAME="${SESSION_NAME:-anchor_ode_v2_tuned_camus_echo}"
OLD_SESSION_NAME="${OLD_SESSION_NAME:-anchor_ode_v2_camus_echo_20260515_123056}"
...
if tmux has-session -t "${OLD_SESSION_NAME}" 2>/dev/null; then
  echo "Stopping old AnchorODE v2 session '${OLD_SESSION_NAME}'."
...
ECHO_CMD="$(echo_cmd 0 anchor_ode_v2_tuned_echo anchor_ode_v2_tuned_echo \
  "${COMMON_ARGS[@]}" \
...
CAMUS_CMD="$(echo_cmd 1 anchor_ode_v2_tuned_camus anchor_ode_v2_tuned_camus \
  "${COMMON_ARGS[@]}" \
  dataset_name=camus \
...
tmux new-session -d -s "${SESSION_NAME}" -n echo "cd '${PROJECT_DIR}' && echo '[\$(date +%F\\ %T)] START anchor_ode_v2_tuned_echo' && ${ECHO_CMD} 2>&1 | tee '${LOG_DIR}/anchor_ode_v2_tuned_echo.log'; echo '[\$(date +%F\\ %T)] END anchor_ode_v2_tuned_echo'; exec bash"
tmux new-window -t "${SESSION_NAME}" -n camus "cd '${PROJECT_DIR}' && echo '[\$(date +%F\\ %T)] START anchor_ode_v2_tuned_camus' && ${CAMUS_CMD} 2>&1 | tee '${LOG_DIR}/anchor_ode_v2_tuned_camus.log'; echo '[\$(date +%F\\ %T)] END anchor_ode_v2_tuned_camus'; exec bash"

echo "Started tmux session '${SESSION_NAME}'."
echo "Attach with: tmux attach -t ${SESSION_NAME}"
echo "Logs:"
echo "  ${PROJECT_DIR}/${LOG_DIR}/anchor_ode_v2_tuned_echo.log"
echo "  ${PROJECT_DIR}/${LOG_DIR}/anchor_ode_v2_tuned_camus.log"
