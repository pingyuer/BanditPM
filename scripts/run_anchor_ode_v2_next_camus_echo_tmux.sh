...
SESSION_NAME="${SESSION_NAME:-anchor_ode_v2_next_camus_echo}"
...
ECHO_CMD="$(echo_cmd 0 anchor_ode_v2_echo_more_guidance anchor_ode_v2_echo_more_guidance \
  "${COMMON_ARGS[@]}" \
...
CAMUS_CMD="$(echo_cmd 1 anchor_ode_v2_camus_trust_guided anchor_ode_v2_camus_trust_guided \
  "${COMMON_ARGS[@]}" \
  dataset_name=camus \
...
tmux new-session -d -s "${SESSION_NAME}" -n echo "cd '${PROJECT_DIR}' && echo '[\$(date +%F\\ %T)] START anchor_ode_v2_echo_more_guidance' && ${ECHO_CMD} 2>&1 | tee '${LOG_DIR}/anchor_ode_v2_echo_more_guidance.log'; echo '[\$(date +%F\\ %T)] END anchor_ode_v2_echo_more_guidance'; exec bash"
tmux new-window -t "${SESSION_NAME}" -n camus "cd '${PROJECT_DIR}' && echo '[\$(date +%F\\ %T)] START anchor_ode_v2_camus_trust_guided' && ${CAMUS_CMD} 2>&1 | tee '${LOG_DIR}/anchor_ode_v2_camus_trust_guided.log'; echo '[\$(date +%F\\ %T)] END anchor_ode_v2_camus_trust_guided'; exec bash"

echo "Started tmux session '${SESSION_NAME}'."
echo "Attach with: tmux attach -t ${SESSION_NAME}"
echo "Logs:"
echo "  ${PROJECT_DIR}/${LOG_DIR}/anchor_ode_v2_echo_more_guidance.log"
echo "  ${PROJECT_DIR}/${LOG_DIR}/anchor_ode_v2_camus_trust_guided.log"
