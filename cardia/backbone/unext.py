...
from .shifted_mlp import ShiftedMLPBlock
from ..memory.helpers import _get_activation


class ConvBlock(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, activation: str = "GELU") -> None:
        super().__init__()
...
class UpBlock(nn.Module):
    def __init__(self, in_dim: int, skip_dim: int, out_dim: int, activation: str = "GELU") -> None:
        super().__init__()
        self.proj = ConvBlock(in_dim + skip_dim, out_dim, activation=activation)
...
class UNeXtBackbone(nn.Module):
    ...
    def __init__(
        ...
        self.stem = ConvBlock(dims[0], dims[0], activation=activation)
        self.down1 = nn.Sequential(nn.MaxPool2d(2), ConvBlock(dims[0], dims[1], activation=activation))
        self.down2 = nn.Sequential(nn.MaxPool2d(2), ConvBlock(dims[1], dims[2], activation=activation))
        self.token_mlp = nn.Sequential(
            ShiftedMLPBlock(dims[2]),
            ShiftedMLPBlock(dims[2], shift=-1),
        )
        self.up1 = UpBlock(dims[2], dims[1], dims[1], activation=activation)
        self.up2 = UpBlock(dims[1], dims[0], dims[0], activation=activation)
        self.full_res = ConvBlock(dims[0], dims[0], activation=activation)
